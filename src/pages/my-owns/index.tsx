...
import { getMyOwns, deleteMyOwnById } from 'apiSdk/my-owns';
import { MyOwnInterface } from 'interfaces/my-own';

type ColumnType = ColumnDef<MyOwnInterface, unknown>;

interface MyOwnListPageProps {
...
export function MyOwnListPage(props: MyOwnListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MyOwnInterface>>(
    () => `/my-owns?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MyOwnInterface) => {
    if (hasAccess('my_own', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/my-owns/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/my-owns/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('my_own', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/my-owns/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('my_own', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
