...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  ...
  'ticket.submit.create': {
    roles: ['tenant-roles', 'tenant-owner-roles'],
    key: 'ticket',
    tenantPath: [],
    userPath: [],
  },
  'project.update.update': { roles: ['tenant-roles'], key: 'project', tenantPath: [], userPath: [] },
};

const ownerRoles: string[] = ['owner'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['owner', 'administrator', 'developer'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.my_own.findFirst({
