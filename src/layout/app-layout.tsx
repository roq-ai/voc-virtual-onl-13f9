...
import { getMyOwns } from 'apiSdk/my-owns';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<MyOwnInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/my-owns` : null),
    () => getMyOwns({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'My Owns',
      path: isTenantUser ? `/my-owns/view/${data?.[0]?.id}` : '/my-owns',
      entity: 'my_own',
